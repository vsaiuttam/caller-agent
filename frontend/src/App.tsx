...
export default function App() {
  ...
  return (
    <div className="flex h-full">
      {/* ── Sidebar ──────────────────────────────────────────── */}
      <aside className="flex w-56 shrink-0 flex-col border-r border-[var(--surface-border)] bg-surface">
        {/* Brand */}
        ...
        <nav className="flex-1 overflow-y-auto px-2 pb-2">
          {NAV_GROUPS.map((group) => (
            <div key={group.heading} className="mb-1">
              <p className="px-3 pb-1 pt-3 text-[10px] font-semibold uppercase tracking-widest text-ink-muted">
                {group.heading}
              </p>
              <div className="space-y-px">
                {group.items.map((item) => (
                  <NavLink
                    ...
                    to={item.to}
                    className={({ isActive }) =>
                      `group relative flex items-center gap-2.5 rounded-lg px-3 py-[7px] text-[13px] transition-colors duration-100 ${
                        isActive
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        {isActive && (
                          <div className="absolute left-0 top-1/2 h-4 w-[2.5px] -translate-y-1/2 rounded-r-sm bg-brand" />
                        )}
                        ...
                        {item.badge === "review" &&
                          (stats?.pending_review ?? 0) > 0 && (
                            <span className="tnum rounded bg-warning/15 px-1.5 py-0.5 text-[10px] font-medium text-warning">
        ...
        {/* Bottom */}
        <div className="space-y-2 border-t border-[var(--surface-border)] px-3 py-3">
          <button
            type="button"
            onClick={() => {
            ...
            className="flex w-full items-center gap-2 rounded-lg px-3 py-[7px] text-[13px] text-ink-secondary transition-colors hover:bg-elevated hover:text-ink"
          >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Header bar */}
        <header className="flex h-11 shrink-0 items-center justify-between gap-4 border-b border-[var(--surface-border)] bg-surface px-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-[var(--surface-border)] bg-elevated px-2 py-1 text-xs text-ink-muted transition-colors hover:border-brand/25 hover:text-ink-secondary"
            >
              <IconSearch size={12} />
